...
function profilListesi(fenomen) {
  const donen = [];
  for (const isim in fenomen) {
    donen.push(fenomen[isim].profile);
  }
...
function fenomenEkle(a,num,profile,followers,posts,platform) {
  const add = {
    "number": num,
    "profile": profile,
    "followers": followers,
...
function enFenomenler(fenomen) {
  const donen = [];
  for (const isim in fenomen) {
    if(fenomen[isim].followers > 100000000){
      donen.push(fenomen[isim].profile);
    }
...
function fenomenGonderimSayisi(fenomen,kisi){
  for (const isim in fenomen) {
    if(fenomen[isim].profile === kisi){
    return fenomen[isim].posts;
    }
  }
}
...
function platformaGoreCokGonderiYapanFenomen(fenomen,yer){
  let postCount = 0;
  let most = "";
  for (const isim in fenomen) {
    
    if(fenomen[isim].platform === yer && fenomen[isim].posts > postCount){
      postCount = fenomen[isim].posts;
      most = fenomen[isim].profile;
